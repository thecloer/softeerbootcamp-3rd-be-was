...
- `ApplicationContainer`와 `UserService`가 꼭 필요할까?
  - `ApplicationContainer`
    ...
    - 컨트롤러, 서비스 또한 `ApplicationContainer`에서 관리하여 후에 객체 생성 방법이 변경되어도 `ApplicationContainer`에서만 수정하면 되도록 객체의 생성과 소비를 분리
  - `UserService`
...
- step 2에서 고민했던 오류 응답 상태코드에 대한 고민
  ...
  - 회원가입 성공 시 201 created 상태코드와 생성한 데이터를 받을 수 있는 URI를 응답 헤더에 포함해 응답 ([참고](https://www.rfc-editor.org/rfc/rfc9110#name-201-created))
